...
\node(e) {\Gamma\PModels \false} [sibling distance = 6.5cm]
	...
	child {node(2) {\Gamma\PModels \forall\beta(P^n(\predTFst[n],\predTSnd[n])\to\false)\to\false}
		child {{node[label={[yshift=-0.44cm]above:\vdots}](21) {}}
			child[xshift=2.5cm] {node(211) {\Gamma\PModels P^1(\predTFst[1],\predTSnd[1])}}
			child[xshift=2.5cm] {node(212) {\Gamma\PModels P^1(\predTFst[1],\predTSnd[1])\to\dots\to\forall\beta(P^n(\predTFst[n],\predTSnd[n])\to\false)\to\false}
