...
\def\dist{0.6cm}
\node(e) {\Gamma_C\cup\conGM\vdash \falses} [sibling distance=2*\dist+3.4cm] 
	child {node(1) {\Gamma_C\cup\conGM\vdash Q_f(a)}}
	child {node(2) {\Gamma_C\cup\conGM\vdash Q_f(a)\to\falses}
		child {node(21) {\Gamma_C\cup\conGM\vdash\forall\alpha(Q_f(\alpha)\to\falses)}}};
