...
\begin{definition}
	Let $B$ be a set and $A\subseteq B$, $\vec{a}=a_1\dots a_n$ for some $n\in\N$ and some $a_1,\dots,a_n\in A$ and $\vec{b}=b_1\dots b_n$ for some $b_1,\dots,b_n\in B$. For an $a\in A$ the substitution of $\vec{a}$ by $\vec{b}$, denoted by $a\left[\vec{a}:=\vec{b}\right]$, is defined as follows.
	\[a\left[a_1\dots a_n:=b_1\dots b_n\right]:=
	\begin{cases}
		a &\text{if $n=0$}\\
		b_n &\text{if $n>0$ and $a_n= a$}\\
		a\left[a_1\dots a_{n-1}:=b_1\dots b_{n-1}\right] &\text{if $n>0$ and $a_n\neq a$}
	\end{cases}\]
\end{definition}
