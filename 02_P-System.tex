...
\def\rm{edge from parent [opacity=0]}
\node(e) {\vdashf \falses} [sibling distance=10.4cm] 
child [sibling distance=4cm] {node(1) {\vdashf Q'(b)} \rm
	child {node(11) {\vdashf S(a,b)} \rm}
	child [sibling distance=3.7cm] {node(12) {\vdashf S(a,b)\to Q'(b)} \rm
		child {node(121) {\vdashf Q(a)} \rm}
		child {node(122) {\vdashf Q(a)\to S(a,b)\to Q'(b)} \rm
			child {node(1221) {\vdashf\forall\alpha\beta(Q(\alpha)\to S(\alpha,\beta)\to Q'(\beta))} \rm}}}}
child {node(2) {\vdashf Q'(b)\to\falses} \rm
	child {node(21) {Q'(b)\vdashf\falses} \rm}};

%the yshift does not work on nodes so we create a coordinate for every node
\foreach \x in {1,2,11,12,21,121,122,1221}{
\coordinate(c\x) at (\x);
}
...
\newlength{\sleft}
\newlength{\sright}
%draw lines for each node with 2 children
\foreach \x in {1,11,121}{
\pgfmathtruncatemacro{\xn}{\x+1}
%calculate with of line
\getwidthofnode{\sleft}{\xn}
\getwidthofnode{\sright}{\x}
%draw line
\draw[shorten >=-\sleft/2, shorten <=-\sright/2] ([yshift=-0.25cm]c\x) -- ([yshift=-0.25cm]c\xn);
}

%draw lines for each node with 1 child
\foreach \x in {21,1221}{
%calculate with of line
\getwidthofnode{\sright}{\x}
%draw line
