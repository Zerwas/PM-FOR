...
\begin{proof} By induction on the length $i$ of the computation.
	...
	For the case that $\boldsymbol{r=+(1,Q')}$, we have that $\widehat{Q}=Q'$, $\widehat{n}=n+1$, and $\widehat{m}=m$. So we need to increment register 1 and ensure that the state of $b$ is $Q'$.
	\begin{figure}[H]
		\centering
