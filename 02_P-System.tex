...
\item $\forall\alpha\beta\gamma\delta(Q(\alpha)\to S(\alpha,\beta)\to R_1(\alpha,\gamma)\to D(\gamma))$ \\prevent zero %TODO necessary?
...
\end{tikzpicture}
\end{figure}
Starting from $Q'(b)\vdashf\false$ we can deduce:
\begin{figure}[H]
\begin{tikzpicture}[grow=up,level distance=0.5cm,
execute at begin node=$, execute at end node=$,
every node/.style={opacity=1},
...
child [sibling distance=3cm] {node(1) {\vdashf P(d,a_0)} 
	child {node(11) {\vdashf R_1(b,d)} }
	child [sibling distance=3.7cm] {node(12) {\vdashf R_1(b,d)\to Q'(b)} 
		child {node(121) {\vdashf R_1(a,a_0)} }
		child [sibling distance=4.5cm] {node(122) {\vdashf R_1(a,a_0)\to R_1(b,d)\to Q'(b)} 
			child {node(1221) {\vdashf S(a,b)} }
			child [sibling distance=5cm] {node(1222) {\vdashf S(a,b)\to R_1(a,a_0)\to R_1(b,d)\to Q'(b)}
...
%set of all positions in the tree
\def\nodes{,1,2,11,12,21,121,122,1221,1222,12221,12222,122221}
%the yshift does not work on nodes so we create a coordinate for every node
\foreach \x in \nodes{
\ifnodedefined{\x}{
	\coordinate(c\x) at (\x);
