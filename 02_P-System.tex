...
\begin{proof} By induction on the length $i$ of the computation.
	...
	From the fact that a final configuration is reachable from $C$ in $i$ steps we can deduce that there exists a configuration $D=\langle P,m',n'\rangle$ such that $C\Rightarrow_M^r D$ for some $r\in\mathcal{R}_\autStates$ and a final configuration is reachable from $D$ in $i'$ steps. We also know that $C=\langle Q,m,n\rangle$ for some $Q\in\autStates\setminus\{Q_f\}$ and some $m,n\in\mathbb{N}$. The basic idea is to deduce $\Gamma_D$ from $\Gamma_C\cup\conGM$ and then apply the induction hypothesis to $\Gamma_D\cup\conGM$.
	\begin{figure}[H]
		\centering
		\input{Figure.Tableau}
	\end{figure}
	
	There are four possible cases for the type of the rule $r$. We will only consider the cases $r=+(1,Q')$ and $r=-(1,Q_1,Q_2)$, the two cases $r=+(2,Q')$ and $r=-(2,Q_1,Q_2)$ follow by exchanging the roles of register 1 and register 2 in the first two cases.
	In both cases we need a new free variable $b$ representing the configuration $D$ and the value in register 2 does not change.
	...
	We first introduce a new variables representing $D$ (let $b\in \VarP\setminus\text{FV}(\Gamma_C\cup\conGM)$).
