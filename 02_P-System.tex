...
\begin{definition}
Let $A$ be a \SysP-formula, and $\Gamma$ be a \SysP-basis. The formula $A$ is \define{derivable} from $\Gamma$, denoted by $\Gamma\PModels A$, if $A$ can be produced using the following deduction rules.
\begin{mdframed}
