...
\end{align*}
Let $M=(\autStates,Q_0,Q_f,\autRules)$ be a two-counter automaton, w.l.o.g. $S,P,R_1,R_2,E,D\notin\autStates$. In the following we will consider \SysP-formulas over $\VarP$ and $\RelP$, where $\RelP=\autStates\uplus\{S,P,R_1,R_2,E,D\}$. We will abbreviate $P(a,a)$ to $P(a)$, note that this way we can use binary predicate symbols as unary ones.
...
\begin{itemize}
	...
	\item Finally $D(a)$ states that ``$a$ is not the end of a chain".
\end{itemize}
...
\begin{itemize}
	...
	\item $D(a_i),D(b_j)~\text{for}~i\in\{0,\dots,m-1\}~\text{and}~j\in\{0,\dots,n-1\}$
	\item $E(a_m),E(b_n)$
\end{itemize}
...
\begin{itemize}
	...
	%TODO necessary?
	\item $\forall\alpha(\forall\beta(R_1(\alpha,\beta)\to\false)\to\false)$\\every element represents a configuration so it has a value for register 1
	\item $\forall\alpha(\forall\beta(R_2(\alpha,\beta)\to\false)\to\false)$\\every element represents a configuration so it has a value for register 2
...
\begin{proof} By induction on the length $i$ of the computation.
	...
	\begin{enumerate}[label=]%TODO is this a good idea?
		\item $R_1(a,a_0)$, $P(a_{i-1},a_i)$ and $D(a_{i-1})$ for $i\in\{1,\dots,m\}$,
		\item $R_2(a,b_0)$, $P(b_{i-1},b_i)$ and $D(b_{i-1})$ for $i\in\{1,\dots,n\}$,
		\item $Q(a)$, $E(a_m)$ and $E(b_n)$.
	\end{enumerate}
	And $\Gamma_D$ contains the formulas:
	\begin{enumerate}[label=]
		\item $R_1(\widehat{a},\widehat{a}_0)$, $P(\widehat{a}_{i-1},\widehat{a}_i)$ and $D(\widehat{a}_{i-1})$ for $i\in\{1,\dots,\widehat{m}\}$,
		\item $R_2(\widehat{a},\widehat{b}_0)$, $P(\widehat{b}_{i-1},\widehat{b}_i)$ and $D(\widehat{b}_{i-1})$ for $i\in\{1,\dots,\widehat{n}\}$,
		\item $\widehat{Q}(\widehat{a})$, $E({\widehat{a}}_{\widehat{m}})$ and $E({\widehat{b}}_{\widehat{n}})$.
	\end{enumerate}
