...
\begin{mdframed}
\begin{align*}
&(\exists\text{-Elimination}) &&\frac{\Gamma\vdashf \forall\alpha(A\to\false)\to\false}{\Gamma\vdashf A\left[ \alpha:=a\right]} &&a\notin\emph{FV}(\Gamma)
\end{align*}
\end{mdframed}
\begin{proof}
Let $I=(\Delta,\cdot^I)$ be a model of $\Gamma$ with $\false^I=\bot$.
\begin{align*}
I\models\Gamma&\Rightarrow I\models\forall\alpha(A\to\false)\to\false\\
&\Rightarrow (\forall\alpha(A\to\false))^I\to\false^I\\
&\Rightarrow (\forall\alpha(A\to\false))^I\to\bot\\
&\Rightarrow \neg(\forall\alpha(A\to\false))^I\\
&\Rightarrow \neg(\forall a\in\Delta:(A\to\false)^{I\left[\alpha:=a\right]})\\
&\Rightarrow \exists a\in\Delta:\neg(A^{I\left[\alpha:=a\right]}\to\false^{I\left[\alpha:=a\right]})\\
&\Rightarrow \exists a\in\Delta:\neg(A^{I\left[\alpha:=a\right]}\to\bot)\\
&\Rightarrow \exists a\in\Delta:\neg(\neg A^{I\left[\alpha:=a\right]})\\
&\Rightarrow \exists a\in\Delta:A^{I\left[\alpha:=a\right]}\\
\end{align*}
\end{proof}
%--------------------------------------------------------------------------------
\begin{mdframed}
...
\begin{lemma}
\begin{align*}
&\text{$M$ terminates on input $(0,0)$}&& \text{iff} &\text{$\Gamma_M\vdash\false$ hold in system P.}
\end{align*}
\end{lemma}
\begin{claim}
...
$$\cfrac{\cfrac{Q'(b)\vdash\falses}
               {\vdash Q'(b)\to\falses}
\hspace{0.4cm}
\cfrac{\cfrac{\cfrac{\vdash \forall\alpha\beta(Q(\alpha)\to S(\alpha,\beta)\to Q'(\beta))}
                    {\vdash Q(a)\to S(a,b)\to Q'(b)}
              \hspace{0.4cm}
              \vdash Q(a)}
             {\vdash S(a,b)\to Q'(b)}
       \hspace{0.4cm}
