...
\begin{itemize}
	...
	\item[] \underline{$M=M_1M_2$} for some $M_1,M_2\in\lambdaTerms$.\\
		...
		Therefore $x=x_A$ for some universal formula $A=\forall\vec{\alpha}(A_1\to\dots\to A_n\to P(\predFst,\predSnd))$ in $\Gamma$ with $A_i=P^i(\predFst[i],\predSnd[i])$ for some $P^i\in\RelP$ and $\predFst[i], \predSnd[i]\in\VarP$ for all $i\in\{1,\dots,n\}$ and $\predVec=\predVec[1]\dots\predVec[m]$ for some $\predVec[1],\dots,\predVec[m]\in\VarP$. %forall ai exists j: ai=aj or ai=bj
