...
\begin{itemize}
	...
	\item[] \underline{$M=M_1M_2$} for some $M_1,M_2\in\lambdaTerms$.\\
		...
		%TODO mapping is bullshit
		It follows that $x=x_A$ and $(x:\trans{A})\in\trans{\Gamma}$ for some universal formula $A=\forall\predVec(P^1(\predFst[1],\predSnd[1])\to\dots\to P^n(\predFst[n],\predSnd[n])\to P(\predFst,\predSnd))$ in $\Gamma$ for some $P^i\in\RelP$ and some $\predFst[i], \predSnd[i]\in\VarP$ for all $i\in\{1,\dots,n\}$, $\predFst,\predSnd\in\VarP$, and $\predVec=\predVec[1]\dots\predVec[m]$ for some $\predVec[1],\dots,\predVec[m]\in\VarP$. %forall ai exists j: ai=aj or ai=bj
		...
		Finally we have to deduce $\Gamma\PModels P(\predTFst,\predTSnd)$.
		\begin{figure}[H]
			\centering
			\input{Figure.C1.3}
		\end{figure}
...
\begin{lemma}
Let $\Gamma$ be a \SysP-basis, $P\in\RelP$, and $\predTFst,\predTSnd\in\lambdaTypes$ such that $\Gamma\PModels P(\predTFst,\predTSnd)$ holds.
Then there exists a term $M\in\lambdaTerms$ such that $\trans{\Gamma}\lambdaModels M: \code{P}{\predTFst}{\predTSnd}$.
\end{lemma}
