...
\[a,\alpha\in\lambdaTypVar;t_1,t_2,t\in\lambdaTypes\uncover<2->{;x\in\lambdaValVar;10M_1,M_2,M\in\lambdaTerms}\]
\lambdaTwo{} Typen
...
\begin{frame}{\lambdaTwo}{Bewohntheitsproblem \lambdaInhab}
...
\begin{frame}{Beweis}{Konstruktion}
	...
	\alt<1>{(\alpha\to p_1)\to(\beta\to p_2)\to p}{\code{P}{\alpha}{\beta}} &\text{falls $A=P(\alpha,\beta)$} 
	\end{cases}
	\]
...
\begin{frame}{Beweis}{$\Leftarrow$-Richtung}
	...
	~
	\begin{center}
	\alt<4-6>{$A=\forall\predVec(P^1(\predFst[1],\predSnd[1])\to\dots\to P^n(\predFst[n],\predSnd[n])\to P(\predFst,\predSnd))$}{\alt<2>{Alle wohlgetypten \lambdaTwo{} Terme sind stark normalisierend.}{\phantom{Alle wohlgetypten \lambdaTwo{} Terme sind stark normalisierend.}}}
	\end{center}
\end{frame}
